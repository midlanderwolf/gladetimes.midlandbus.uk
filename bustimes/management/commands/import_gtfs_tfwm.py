...
class Command(BaseCommand):
    ...
    def handle_zipfile(self, path):
        ...
        # Determine trips DataFrame safely
        if hasattr(feed.trips, "df"):
            trips_df = feed.trips.df
        elif hasattr(feed.trips, "data"):
            trips_df = feed.trips.data
        else:
            trips_df = feed.trips

        for line in trips_df.itertuples():
            route = self.routes[line.route_id]

            # Safely get direction_id, default to 0 if missing or None
            direction = getattr(line, "direction_id", 0)
            ...
            if pd.notna(headsign := line.trip_headsign):
                ...
                if headsign:
                    ...
                    # Use direction safely here too
                    headsigns[line.route_id][direction].add(headsign)
        ...
        for line in feed.stop_times.itertuples():
            ...
            if stop := stops.get(line.stop_id):
                stop_time.stop = stop
            elif (stop := stops_not_created.get(line.stop_id)) is not None:
    ...
    def handle(self, *args, **options):
        collections = DataSource.objects.filter(
            url__startswith="http://api.tfwm.org.uk/gtfs/"
        )
